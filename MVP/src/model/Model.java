...
public interface Model{
	...
	/**
	 * This method saves a all the solutions of the mazes to the file
	 * so it could be used next time
	 */
	void saveSolutionToFile();
	/**
	 * This method loads a all the solutions of the mazes from the file
	 * the Solutions were saved in past games
	 */
	void loadSolutionFromFile();
	/**
	 * This method returns the floor of the maze(X) in a format of int[][]
	 * @param name The name of the maze how it is saved in Model
	 * @param height the floor index
	 * @return a representation of the board as int[][]
	 */
	int[][] getMazeCrossByHeight(String name, int height);
	/**
	 * 
	 * @param name The name of the maze how it is saved in Model
	 * @return a representation of the board as int[][][] but the indexes with extra meaning(not only 1/0)
	 */
